...
# use d1a    filter(harvest == "6_6" & season == "2023") %>% 
    group_by(harvest, season, wall, point, spp) %>%  #combines origin into spp for simple summary tables
    summarize(
...
d1_stub <- d1 %>%   #d1a is useful to test calculation from the mostly raw data
    ...
    filter(harvest == "6_6") %>% 
    filter(all_sizes > 0) %>% 
    arrange(desc(season), harvest, wall, point, type, spp)
...
#    filter(season == "2023") %>%  # filter for season while checking code, then run for all seasons
    group_by(harvest, season, wall, point, type) %>%  # include "brush"
    summarize(
        trmt = first(trmt),
        point_count = n_distinct(point),
