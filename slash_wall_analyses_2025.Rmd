...
forest_type_df <- veg_data %>%
    group_by(cluster, harvest, season, trmt, wall, brush, forest_type) %>% 
    count(cluster, harvest,season, trmt, forest_type) %>% 
    ungroup()
```
...
```{r}
d0 <- veg_data %>% 
    left_join(denom_stand_trmt, by = c("harvest","forest_type", "trmt", "brush")) %>% 
        mutate(
        GTR5ft = exp_sd5_1 + exp_sp_1_5, #already calculated as "upperexposed"
#        total = supB + supC1 + supC2 + supP + expB + expC1 + expC2 + expP, variable already exists as "stemcount"
   ...
   #      TRUE ~ type,  # Keeps the existing value of "type" for all other cases
   #  )) %>%     
    arrange(cluster, harvest, season, trmt, wall, point, spp) %>% 
    select( -upperexposed, -stemcount, -total, -GTR5ft, -all_sizes) #not sure why these are removed. Recalculated in df=d1.1
...
# use d1a    filter(harvest == "6_6" & season == "2023") %>% 
    ...
    summarize(
        ...
        seedA = sum(seedA),
        sup_sd12_5 = sum(sup_sd12_5),    #supB = sum(supB),
#        supC1 = sum(supC1), C! and C2 don't exist in 2023 data
#        supC2 = sum(supC2),
        sup_sd5_1 = sum(sup_sd5_1),   # supD = sum(supD),
        sup_sp_1_5 = sum(sup_sp_1_5),  #supP = sum(supP),
        exp_sd12_5 = sum(exp_sd12_5), #expB = sum(expB),
#        expC1 = sum(expC1),
#        expC2 = sum(expC2),
...
numeric_vars1 <- c("GTR1ft", "all_sizes", "GTR5ft", "seedA", "sup_sd12_5", 
...
#    filter(season == "2023") %>%  # filter for season while checking code, then run for all seasons
    group_by(cluster, trmt, harvest, season, brush, type) %>%  # include "brush"
    summarize(
        wall = first(wall),
        growingseasons = first(growingseasons),
        point_count = n_distinct(point),
       ...
       stems_per_ramet = if_else(sum(stems_per_ramet > 0, na.rm = TRUE) > 0,
        ...
        select(filename, harvest, season, trmt, brush,  type,point_count, denominator, type_count, 
           richness_type_all_sizes, richness_type_GTR1ft, richness_type_GTR5ft, all_sizes,  GTR1ft, GTR5ft,  
           growingseasons, clumps_per_acre, stems_per_ramet, everything()) %>% 
        arrange(desc(season), harvest, trmt, type, brush)
    #arrange(desc(season), harvest, trmt, type, brush, spp, wall,  point)
...
wall_type_occupancy <- type_summary %>%
  group_by(cluster, harvest, season, trmt, wall, type) %>%  #include "brush"
  summarize(
  ...
  mutate(
        filename = "slash_wall_analyses_2025.Rmd",
        df = "wall_type_occupancy"
  ) %>% 
    select(filename, df, cluster, harvest, season, trmt, wall, growingseasons, type, point_count,  plots_with_any, occup_any_pct, plots_with_GTR1ft, 
           GTR1ft_pct, plots_with_GTR5ft, GTR5ft_pct, clumps_per_acre, stems_per_ramet, everything())
```


```{r}
output_file <- file.path(output_folder, paste0("type_occupancy_x_wall_x_trmt_x_season(15sept2025)", ".xlsx"))
...
#species richness (list of species, no quantitative)
use next chunk
```{r eval=FALSE}
richness <- species_by_wall %>% 
    group_by(harvest) %>% 
    filter(year == max(year)) %>% # retain the most recent growing season within each harvest
    ungroup() %>% 
    select(filename, harvest, season, trmt, wall, growingseasons, spp,
           type, point_count, numb_pts_any, numb_pts_GTR1ft, numb_pts_GTR5ft,
           any_size_pct, GTR1ft_pct, GTR5ft_pct, seedA, all_sizes, GTR1ft, GTR5ft) %>% 
 group_by(harvest, season, trmt, growingseasons) %>% 
  summarize(
    filename = first(filename),
    df = "richness",

    richness_any        = sum(numb_pts_any > 0, na.rm = TRUE),
    spp_any             = paste(sort(unique(spp[numb_pts_any > 0])), collapse = ", "),

    richness_GTR1ft     = sum(numb_pts_GTR1ft > 0, na.rm = TRUE),
    spp_GTR1ft          = paste(sort(unique(spp[numb_pts_GTR1ft > 0])), collapse = ", "),

    richness_GTR5ft     = sum(numb_pts_GTR5ft > 0, na.rm = TRUE),
    spp_GTR5ft          = paste(sort(unique(spp[numb_pts_GTR5ft > 0])), collapse = ", "),

    richness_any_pct    = sum(any_size_pct > 0, na.rm = TRUE),
    spp_any_pct         = paste(sort(unique(spp[any_size_pct > 0])), collapse = ", "),

    richness_GTR1ft_pct = sum(GTR1ft_pct > 0, na.rm = TRUE),
    spp_GTR1ft_pct      = paste(sort(unique(spp[GTR1ft_pct > 0])), collapse = ", "),

    richness_GTR5ft_pct = sum(GTR5ft_pct > 0, na.rm = TRUE),
    spp_GTR5ft_pct      = paste(sort(unique(spp[GTR5ft_pct > 0])), collapse = ", "),

    richness_all_sizes  = sum(all_sizes > 0, na.rm = TRUE),
    spp_all_sizes       = paste(sort(unique(spp[all_sizes > 0])), collapse = ", "),

    richness_GTR1ft_all = sum(GTR1ft > 0, na.rm = TRUE),
    spp_GTR1ft_all      = paste(sort(unique(spp[GTR1ft > 0])), collapse = ", "),

    richness_GTR5ft_all = sum(GTR5ft > 0, na.rm = TRUE),
    spp_GTR5ft_all      = paste(sort(unique(spp[GTR5ft > 0])), collapse = ", "),
