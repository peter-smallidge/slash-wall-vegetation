...
forest_type_df <- veg_data %>%
    group_by(harvest, season, trmt, wall, brush, forest_type) %>% 
    count(harvest,season, trmt, forest_type)
```
...
```{r}
d0 <- veg_data %>% 
    left_join(denom_stand_trmt, by = c("harvest", "trmt", "brush")) %>% 
        mutate(
...
# use d1a    filter(harvest == "6_6" & season == "2023") %>% 
    group_by(harvest, season, wall, brush, point, spp) %>%  #combines origin into spp for simple summary tables
    summarize(
...
#    filter(season == "2023") %>%  # filter for season while checking code, then run for all seasons
    group_by(trmt, harvest, season, brush, type) %>%  # include "brush"
    summarize(
        growingseasons = first(growingseasons),
        point_count = n_distinct(point),
...
wall_type_occupancy <- type_summary %>%
  group_by(harvest, season, trmt, wall, type) %>%  #include "brush"
  summarize(
    point_count = n_distinct(point),# Total points in the wall
  ...
  mutate(
    ...
    select(filename, harvest, season, trmt, wall, growingseasons, type, point_count,  plots_with_any, occup_any_pct, plots_with_GTR1ft, 
           GTR1ft_pct, plots_with_GTR5ft, GTR5ft_pct, clumps_per_acre, stems_per_ramet, everything())
```


```{r}
output_file <- file.path(output_folder, paste0("type_occupancy_x_wall_x_trmt_x_season(1august2025)", ".xlsx"))
...
#  filter(season == "2023") %>%  # filter for season while checking code, then run for all seasons
  ...
  mutate(
    ...
    select(filename, df, harvest, season, trmt, wall, growingseasons, spp, type, 
...
output_file <- file.path(output_folder, paste0("spp_x_wall_x_trmt_x_season(8august2025)", ".xlsx"))
...
#    select(-x1, -x2, -supC1, -supC2, -expC1, -expC2, -denom) %>% 
        select(filename, df, harvest, season, trmt,  wall_count, points_per_harvest, growingseasons, 
