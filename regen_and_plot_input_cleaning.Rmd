...
diverse <- regen.per.acre %>% 
  ...
  # does not include commercial spp to avoid overestimating total stems
  # per acre
...
plot.data_full <- 
  left_join(plot.data_05, pre_interfere_stocking,     by = "point") %>% 
   select(!c(harvest.x.x, location.x.x, harvest.y, location.y)) %>% 
  rename("harvest" = "harvest.x", "location" = "location.x", "seedling_numb" = "seedling-numb") %>% 
