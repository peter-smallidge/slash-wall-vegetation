...
#There are 248 plots, but 242 regen points that had observations in 2019. 6 are excluded because
...
tidy_regen_point <- 
  ...
  mutate(
    comm_stock = "no",
    pre_comm_stocking = "no",
    diver_stock = "no",
    pre_diver_stock = "no"
  )

#for stocking breaks, assess heights for deer impact = 0 or 1
#for deer impact=1, there should be 15 stems less than 5 ft per 6 ft plot = 5775
#for deer impact=2, there should be 30 stems less than 5 ft per 6 ft plot = 11550
...
comm_stocking <- tidy_regen_point %>% 
  ...
  mutate(comm_stock = cut(comm_per_acre, breaks=c(0, 770, 1000000),labels=c("no","stocked")))
...
pre_comm_stocking <- tidy_regen_point %>% 
  ...
  mutate(pre_comm_stock = 
           cut(pre_comm_per_acre, breaks=c(0, 5775, 11550, 1000000),labels=c("no","almost", "stocked")))
...
diver_stocking <- tidy_regen_point %>% 
  ...
  mutate(diver_stock = cut(diver_per_acre, breaks=c(0, 770, 1000000),labels=c("no","stocked")))
...
pre_diver_stocking <- tidy_regen_point %>% 
  ...
  mutate(pre_diver_stock = 
           cut(pre_diver_per_acre, breaks=c(0, 5775, 11550, 1000000),labels=c("no","almost", "stocked")))
...
interfere_stocking <- tidy_regen_point %>% 
  ...
  mutate(interfere_stock = cut(interfere_per_acre, breaks=c(0, 770, 1000000),labels=c("no","stocked")))

pre_interfere_stocking <- tidy_regen_point %>% 
  filter(type %in% c("interfering")) %>% 
  filter(size %in% c("exp_short")) %>% 
  group_by(harvest, location, point) %>% 
  summarize(
    pre_interfere_per_acre = sum(stems.per.acre)
  ) %>% 
  mutate(pre_interfere_stock = 
           cut(pre_interfere_per_acre, 
               breaks=c(0, 5775, 11550, 1000000),labels=c("no","almost", "stocked")))
...
plot.data_full <- 
  left_join(plot.data_04, pre_interfere_stocking,     by = "point") %>% 
   select(!c(harvest.y, location.y)) %>% 
  rename("harvest" = "harvest.x", "location" = "location.x") %>% 
  arrange(harvest, location, point)
  
write_csv(plot.data_full, "tidy_plot_2019.csv")
```
