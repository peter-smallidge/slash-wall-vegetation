...
# 18july2025 data for each size class of the df are expressed as "per acre" values. These may need to be adjusted before calculating clump size
...
point_labels_unique <- point_labels %>%
...
cafri2023_structure_wall_summary <- cafri2023_structure_point_summary %>% # use the df = point summary which is filtered for "in" and then average by point
    ...
    select(file, df, harvest, wall, trmt, height, brush, stems_acre_2023, ba_acre_2023, x_dbh, residual_target) %>% 
    mutate_if(is.numeric, round, 0)
...
basalarea2023 <- plot2023_overstory %>% 
    ...
    mutate(across(c(ba_acre, stems_acre), ~ replace_na(.,0))) %>% 
...
output_file <- file.path(output_folder, paste0("site_data_2019_2024(18july2025)", ".xlsx"))
...
Merge veg data, assign attributes, (NO) average stems.ramet, and convert to per acre values at each point
#df=merge2019_2022_b
```{r}
...
regen2023_updated <- regen2023 %>% 
    left_join(point_labels, by = c("harvest", "wall", "location", "trmt", "point", "harvest_original", "forest_type",
                                   "brush")) %>% 
    rename(
        stems.ramet = avg_ramet_size,
        clumps.acre = clump_ac
    ) %>% 
    select(-clump_ac_small, -clump_ac_med, -clump_ac_large, -clump_ac_sapl, -overstory, -stemcount, -exposed, -residual, 
           -filename, -df) %>% 
    left_join(seedlings2019_2024, by =  c("harvest", "season", "wall", "point", "spp", "origin")) %>% 
        mutate(
            seedA = coalesce(seedA.x, seedA.y)
