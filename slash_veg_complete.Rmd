...
file_path_2024 <- "C:\\Users\\pjs23\\Documents\\R\\slash-wall-vegetation\\raw_data\\slash.wall.data_2024_FINAL_1june2025.xlsx"
    # the FINAL 2024 growing season vegetation data added to main folder on 1june2025
...
```{r}
point_labels <- read_excel(file_path_points, sheet = "Sheet1") %>% 
...
regen2024 <- read_excel(file_path_2024, sheet = "regen", skip = 2 ) %>% 
    ...
    mutate(
    numb.per.clump = if_else(origin == "clump",  #TODO need to double check this calculation #DONE, calculation confirmed 2/13/2025
...
# use this df to merge the seedling data 4"to 12" with the plot attribute data. See line 785
# or search for the df name
regen2024seedl <- regen2024 %>% 
    mutate(
        comm_sdl = supA + expA
    ) %>% 
    select(harvest, point, type, spp, comm_sdl) %>% 
    filter(type == "commercial") %>% 
...
#merge the seedlings 4" to 12" within df=regen2024seedl with site attribute data into df=plot2024
# on line number 554, only type = "commercial" were retained
plot2024 <- left_join(plot2024_orig, regen2024seedl, by = c("harvest", "point")) %>%  
...
site_data <- bind_rows(plot2019, plot2020, plot2021, plot2022, plot2023) %>% 
    ...
    mutate( #this might replace "basalarea" in df=plot2024
...
#Do this before the values are converted to a "per acre" basis in the next code chunk
    mutate(
