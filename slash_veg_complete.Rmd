...
file_path_2024 <- "C:\\Users\\pjs23\\Documents\\R\\slash-wall-vegetation\\slash.wall.data_2024_FINAL.xlsx"
    # the 2024 growing season vegetation data added to main folder on 10april2025
    # 
...
file_path_2024_plot <- "C:\\Users\\pjs23\\Documents\\R\\slash-wall-vegetation\\slash.wall.data_2024_FINAL.xlsx"
    # the 2024 growing season plot data added on 10april2025
    # I'm not certain this file path is needed given "file_path_2024" which includes worksheets for "regen" and "plot"
...
# I'm sure there is a more graceful way to code this, without creating two df and then binding them back together.
clump2019 <- raw2019 |> 
    filter(origin == "clump") |> 
    mutate(
        count = 1,
        numb.per.clump = rowSums(across(c(starts_with("sup"), starts_with("exp")), ~ if_else(. > 0, 1, 0)), na.rm = TRUE)
    )
...
#    filter(is.na(date))
  #   mutate(               ## it's unclear why this code is here.  It isn't needed as there are no missing values
...
regen2024 <- read_excel(file_path_2024, sheet = "regen", skip = 2 ) %>% 
    ...
    rename(
        ...
        supD = sup.seed03, # 60" to 1"dbh, radius = 6 ft
        supP = sup.sapl, # 1" to 5" dbh, radius = 16 ft
        expA = exp.seed01,
        expB = exp.seed02,
        expD = exp.seed03,
        expP = exp.sapl
    ) %>%   #  need to check this coded, added on 4/16/2025 but not tested
...
# 
# data entry had created points in some of the cafri walls
# 
# 
