...
file_path_point_labels <- "C:\\Users\\pjs23\\Documents\\R\\slash-wall-vegetation\\summary output\\point_labels_all(readme).xlsx"
...
output_folder_raw <- "C:\\Users\\pjs23\\Documents\\R\\slash-wall-vegetation\\raw_data\\"
...
#    filter(!harvest =="recknagel_north") %>% 
    ...
    mutate(harvest = case_when(
      harvest == "gasline" ~ "gas_line",  #this changes "harvest" from gasline to gas_line
      harvest == "recknagel_south" ~ "recknagel",
   ...
   select(-wall) %>% 
   select(harvest,  season, date, point, spp, origin, count, everything()) %>% 
...
point_labels <- read_excel(file_path_point_labels, sheet = "Sheet1", skip = 0) 
...
```{r}
walls01 = left_join(input_veg01, point_labels, by = c("harvest", "point")) %>% 
    filter(harvest %in% c("gas_line", "campridge", "recknagel", "recknagel_north")) 

table(walls01$harvest, walls01$trmt)
table(walls01$harvest, walls01$location)
table(walls01$trmt, walls01$location)
```
...
```{r}
#this df includes harvest - 6_6, 6_9, and campridge
...
# not needed. Left join added to input_veg02
cafri01 = left_join(input_veg02, location_by_point, by = c("harvest", "point")) |> 
...
#not needed.  data frame configured within input_veg02
...
merge vegetation input files for raw 2023 veg data
```{r}
## merge the datafiles and retain the regen data

veg2023_raw = bind_rows(walls01, input_veg02) |> 
    select(harvest, wall, season, date, trmt, location, point, spp, origin, count,
        sup.seed01, sup.seed02, sup.seed03, sup.sapl,
        exp.seed01, exp.seed02, exp.seed03, exp.sapl,
        harvest_original, forest_type, brush
        ) |> 
mutate(spp = fct_recode(spp, 
    ...
    arrange(harvest, wall, point) %>% 
    mutate(
        filename = "test_2023_regen_plot_input.Rmd",
        df = "veg2023_raw"
) %>% 
    select(filename, df, harvest, wall, season, date, trmt, location, point, spp, origin, count, everything())
 ...
 #     test_clump = mean(count)  
     ...
     # this should be the average number of stems per clump in each stand. 
     #Note the older harvests have a lower average number
 

table(data01$harvest, data01$trmt)
table(data01$harvest, data01$location)
table(data01$trmt, data01$location)

test_output <- data01 |> 
    filter(harvest == "6_6" & wall == "11")
...
output_file <- file.path(output_folder_raw, paste0("veg2023_raw(22july2025)", ".xlsx"))
write_xlsx(veg2023_raw, output_file)
...
select(
  ...
  exp.seed01, exp.seed02, exp.seed03, exp.sapl, multip01, multip02_03, multip_sapl)
) %>% 
#mutate(spp = fct_relevel(spp, sort(levels(spp)))) |>
mutate(
    filename = "test_2023_regen_plot_input.Rmd",
    df = "veg2023_peracre_calc"
) %>% 
arrange(harvest, wall, trmt, origin, spp, origin, point) %>% 
select(filename, df, harvest, wall, location, trmt, point, spp, origin, everything())

output_file <- file.path(output_folder, paste0("veg2023_peracre(19july2025)", ".xlsx"))
write_xlsx(veg2023_peracre_calc, output_file)
