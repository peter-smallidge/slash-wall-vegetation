...
point_count_old_walls <- read_csv("count_of_points_harvest_x_treatment.csv") |> 
    rename(
        stand = harvest,
        status = wall
    ) |> 
...
wall_denominator <- read.csv("count_of_points_stand_by_protection.csv") |> 
...
# mutate(clumped = origin) %>%  # character variable "clumped" will allow for the enumeration of the number of clumps per acre
...
# mutate(clumped = as.numeric(clumped)) %>%  # variable clumped is numeric with 0 = single stem and 1 = clump of stems
...
mutate(
  ...
  mutate(                 
    ...
    clump_ac_01 = case_when(
      ...
      #when origin=clump, calculate the clump/ac as numeric. Else, change clump/ac=NA
      TRUE ~ NA_real_
    ),
...
arrange(stand, wall, status, point, spp, origin) %>% 
...
summarize(
  ...
  avg_ramet_size = mean(ramet, na.rm = TRUE)  #count is the numnber of stems per rament
) %>% 
mutate(
type = "empty"
) |> 
...
#
# the clumps per acre at the point level (df = veg2023summ) is calculated correctly
# 
...
veg2023summ$type[veg2023summ$spp %in% c("app", "bta", "haw", "brs", "la", "ns", "pb", "qa", "rp", "scp", "smc", "svb", "wh", "wi" )] <- "diversity" #n=14
...
# find_distinct_2 <- point_summed |> 
#    select(stand, wall, status, point, spp) %>%
#   summarize(count = n()) |> 
#   filter(count > 1)
