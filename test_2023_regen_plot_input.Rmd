...
location_by_point <- read_excel(file_path_point_labels, sheet = "Sheet1", skip = 0) 
...
```{r}
walls01 = left_join(input_veg01, location_by_point, by = c("harvest", "point")) |> 
    filter(harvest %in% c("gasline", "campridge", "recknagel", "recknagel_north")) 
...
input_veg02 <- read_excel("slash.wall.data.2023.23july2024(maybe FINAL).xlsx", 
   ...
   select(-wall) %>% 
   select(harvest, season, date, point, spp, origin, count, everything()) %>% 
    left_join(location_by_point, by = c("harvest", "point"))
...
data01 = bind_rows(walls01, input_veg02) |> 
...
output_file <- file.path(output_folder, paste0("veg2023raw(27june2025)", ".xlsx"))
write_xlsx(veg2023raw, output_file)
